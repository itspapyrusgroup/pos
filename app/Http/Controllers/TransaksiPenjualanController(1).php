...
class TransaksiPenjualanController extends Controller
{
    ...
    public function simpan(Request $request)
    {
        ...
        try {
            $result = DB::transaction(function () use ($validated, $transactionAt) {
                ...
                if ($koInput !== '') {
                    ...
                    if ($ko && $ko->pesanan_penjualan_id) {
                        ...
                        if (in_array((string) $pesanan->status_pembayaran, ['CANCELLED', 'VOID'], true)) {
                            ...
                            $this->appendToExistingOrder($pesanan, $validated, $itemPayload, $paidTambahan, $promoDiskon, $currentUserId, (int) $shiftKasir->id, $transactionAt, $validated['existing_items'] ?? [], $removeOtp);
                ...
                $pesananBaru = $this->createNewOrder($validated, $templateHargaId, $itemPayload, $paidTambahan, $koInput, $promoDiskon, $currentUserId, (int) $shiftKasir->id, $transactionAt);
                $this->markPromoAsUsed($validated['promo_sumber'] ?? null, $validated['promo_kode'] ?? null);
    ...
    private function appendToExistingOrder(PesananPenjualan $pesanan, array $validated, array $itemPayload, float $paidTambahan, float $promoDiskon, ?int $currentUserId, ?int $shiftKasirId, Carbon $transactionAt, array $existingItemsRequest = [], ?PenjualanVoidOtp $removeOtp = null): void
    {
        ...
        foreach ($itemPayload as $item) {
            ...
            $this->applyStockMutationForItem($item, (int) $validated['cabang_id'], (int) $pesanan->id, $transactionAt, 'DRAFT'); // Force DRAFT to move to on_order
        }
        ...
        foreach ($existingItemsRequest as $reqItem) {
            ...
            if ($existingItem && (float) $reqItem['qty'] !== (float) $existingItem->qty) {
                ...
                if ($diffQty > 0) {
                    $this->applyStockMutationForItem($itemMutation, (int) $validated['cabang_id'], (int) $pesanan->id, $transactionAt, 'DRAFT');
                } else if ($diffQty < 0) {
        ...
        $netPerubahanTotal = (float) $totalTambahan;
        ...
        if ($paidBaru > 0 && $balanceBaru > 0) {
            ...
            foreach ($allItems as $fItem) {
               if ($fItem->produk_id) {
                   ...
                   if ($produk && $produk->track_stok) {
                       $this->finishStokOnOrder($produk->id, (int) $validated['cabang_id'], (float) $fItem->qty, (int) $pesanan->id, $transactionAt);
                   }
                   ...
                   if ($paket) {
                       foreach ($paket->items as $paketItem) {
                           ...
                           if ($produkBom && $produkBom->track_stok) {
                               ...
                               $this->finishStokOnOrder($produkBom->id, (int) $validated['cabang_id'], $qtySelesai, (int) $pesanan->id, $transactionAt);
                           }
                       }
                   }
               }
    ...
    private function createNewOrder(array $validated, ?int $templateHargaId, array $itemPayload, float $paidAwal, string $koInput, float $promoDiskon, ?int $currentUserId, ?int $shiftKasirId, Carbon $transactionAt): PesananPenjualan
    {
        $pelanggan = $this->upsertPelanggan($validated);
        ...
        foreach ($itemPayload as $item) {
            ...
            $this->applyStockMutationForItem($item, (int) $validated['cabang_id'], (int) $pesanan->id, $transactionAt, $statusPembayaranAwal);
        }
    ...
    private function applyStockMutationForItem(array $item, int $cabangId, int $pesananId, Carbon $transactionAt, string $statusPembayaran = 'PAID'): void
    {
        if ($item['jenis_item'] === 'PRODUK' && $item['produk_id']) {
            $produk = Produk::query()->find($item['produk_id']);
            if ($produk && $produk->track_stok) {
                $this->kurangiStok($produk->id, $cabangId, (float) $item['qty'], $pesananId, $transactionAt, $statusPembayaran);
            }
        }
            ...
            if ($paket) {
                foreach ($paket->items as $paketItem) {
                    ...
                    if ($produkBom && $produkBom->track_stok) {
                        ...
                        $this->kurangiStok($produkBom->id, $cabangId, $qtyKeluar, $pesananId, $transactionAt, $statusPembayaran);
                    }
                }
            }
        }
    ...
    private function kurangiStok(int $produkId, int $cabangId, float $qtyKeluar, int $referensiId, Carbon $transactionAt, string $statusPembayaran = 'PAID'): void
    {
        ...
        $allowNegative = $this->allowMinusStockByCabang($cabangId);
    ...
    private function finishStokOnOrder(int $produkId, int $cabangId, float $qtySelesai, int $referensiId, Carbon $transactionAt): void
    {
        if ($qtySelesai <= 0) {
            return;
        }
