...
<head>
    <style>
        ...
        .summary-wrap { width: 320px; margin-left: auto; margin-top: 10px; }
...
@php($sisa = max((float) $faktur->total - (float) $faktur->dibayar, 0))
    <div class="company">
    ...
    <table class="head">
        <tr>
            ...
            <td style="width: 45%;">
                ...
                <table class="meta">
                    <tr><td>Nomor</td><td>: {{ $faktur->nomor_faktur }}</td></tr>
                    <tr><td>No PO</td><td>: {{ $faktur->pesananPembelian->nomor_po ?? '-' }}</td></tr>
                    <tr><td>Tanggal</td><td>: {{ \Carbon\Carbon::parse($faktur->tanggal_faktur)->format('d M Y') }}</td></tr>
                    <tr><td>Jatuh Tempo</td><td>: {{ $faktur->jatuh_tempo ? \Carbon\Carbon::parse($faktur->jatuh_tempo)->format('d M Y') : '-' }}</td></tr>
                    <tr><td>Status</td><td>: {{ $faktur->status }}</td></tr>
                    <tr><td>Dibuat Oleh</td><td>: {{ $faktur->pembuat->name ?? auth()->user()?->name ?? '-' }}</td></tr>
                </table>
            </td>
        </tr>
    </table>
    ...
    <div class="summary-wrap">
        <table class="summary">
            <tr><td>Sub Total</td><td>{{ number_format((float) $faktur->total, 0, ',', '.') }}</td></tr>
            <tr><td>Sudah Dibayar</td><td>{{ number_format((float) $faktur->dibayar, 0, ',', '.') }}</td></tr>
            <tr><td>Sisa Tagihan</td><td>{{ number_format($sisa, 0, ',', '.') }}</td></tr>
            <tr><td><strong>Total</strong></td><td><strong>{{ number_format((float) $faktur->total, 0, ',', '.') }}</strong></td></tr>
        </table>
    </div>

    <div style="margin-top:10px;"><strong>Keterangan:</strong> {{ $faktur->catatan ?: '-' }}</div>

    <div class="sign">
        <div>Bagian Pembelian</div>
        <div class="space"></div>
        <div><strong>{{ $faktur->pembuat->name ?? auth()->user()?->name ?? '-' }}</strong></div>
    </div>
</body>
</html>
