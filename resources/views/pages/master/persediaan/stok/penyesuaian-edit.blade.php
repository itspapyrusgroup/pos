    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Edit #{{ $penyesuaian->id }}</li>
            </ol>
        </nav>
    </div>
</div>
...
<form method="POST" action="{{ route('persediaan.stok.penyesuaian.update', $penyesuaian) }}" id="penyesuaianForm">
    @csrf
    @method('PUT')
        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" name="catatan" value="{{ old('catatan', $penyesuaian->catatan) }}" placeholder="Opsional">
                </div>
            </div>
        </div>
    </div>
                ...
                <tbody id="penyesuaianBody">
                    @foreach($selectedRows as $idx => $row)
                        <tr data-produk-id="{{ $row['id'] }}">
                            ...
                            <td>
                                <input type="number" step="1" min="0" class="form-control text-end" name="target_qty[{{ $row['id'] }}]" value="{{ (int) old('target_qty.' . $row['id'], $row['target_qty']) }}">
                            </td>
                            <td>
