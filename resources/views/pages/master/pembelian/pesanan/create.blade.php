    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Buat PO</li>
            </ol>
        </nav>
    </div>
</div>
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('pembelian.pesanan.store') }}">
            @csrf
            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label">Nomor PO</label>
                    <input type="text" class="form-control" value="{{ $nomorPo }}" readonly>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Tanggal PO</label>
                    <input type="date" class="form-control" name="tanggal_po" value="{{ old('tanggal_po', date('Y-m-d')) }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Tanggal Kirim</label>
                    <input type="date" class="form-control" name="tanggal_kirim" value="{{ old('tanggal_kirim') }}">
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select js-pemasok-select" name="pemasok_id" data-placeholder="Pilih Pemasok" required>
                        ...
                        @foreach($pemasokList as $pemasok)
                            <option value="{{ $pemasok->id }}" {{ (string) old('pemasok_id') === (string) $pemasok->id ? 'selected' : '' }}>{{ $pemasok->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select js-cabang-select" name="cabang_id" data-placeholder="Pilih Cabang" required>
                        ...
                        @foreach($cabangList as $cabang)
                            <option value="{{ $cabang->id }}" {{ (string) old('cabang_id') === (string) $cabang->id ? 'selected' : '' }}>{{ $cabang->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                ...
                <table class="table table-bordered" id="tabel-item">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td><button type="button" class="btn btn-sm btn-outline-danger btn-hapus-item" disabled>Hapus</button></td>
                        </tr>
                    </tbody>
                </table>
